import Button from '../../ShelfScreen/BookForm/Button/Button'
import './ReadingCounter.css'

const ReadingCounter = ({pagesToday, onDecrease, onIncrease, onReset}) => {
    return (
        <div className="session-card">
            ...
            <div className="counter-row">
                <button className="counter-btn" onClick={onDecrease} disabled={pagesToday === 0}>−</button>
                <span className="counter-value">{pagesToday}</span>
                <button className="counter-btn" onClick={onIncrease}>+</button>
            </div>
            <Button className="ghost counter-reset" onClick={onReset}>Сбросить счётчик</Button>
        </div>
    )
}
